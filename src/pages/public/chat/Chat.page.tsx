...
interface VirtualKeyboard {
  ...
  addEventListener(event: 'geometrychange', handler: () => void): void;
  removeEventListener(event: 'geometrychange', handler: () => void): void;
}
  ...
  useEffect(() => {
    if ('virtualKeyboard' in navigator) {
      ...
      const handleGeometryChange = () => {
        setIsVirtualKeyboard(keyboard.boundingRect.height > 0);
        setViewportHeight(window.innerHeight - keyboard.boundingRect.height);
        scrollToBottom(true);
      };
  ...
  return (
    <div
      className={`flex flex-col fixed inset-x-0 bg-background ${
        isVirtualKeyboard || keyboardVisible
          ? 'bottom-0'
          : 'bottom-8'
      } ${isFocusMode ? 'top-8' : 'top-14'}`}
      style={{
        height: (isVirtualKeyboard || keyboardVisible) ? `${viewportHeight}px` : 'auto',
      }}
    >
      ...
      />
      <ChatInput status={status} onSendMessage={handleSendMessage} keyboardVisible={keyboardVisible} />
    </div>
  );
};
